...
/**
 * CurrentDirectionLayer Component
 * Renders current direction as vector arrows on a Mapbox map
 */
const CurrentDirectionLayer = ({
  map,
  data = [],
  isVisible = false,
  vectorScale = 0.009,
  colorBy = 'speed',
  depthFilter = null,
  displayParameter = 'Current Direction',
  maxVectors = 1000,
  minMagnitude = 0,
  onError = null
}) => {
  const layerIdRef = useRef('current-direction-vectors');
  const sourceIdRef = useRef('current-direction-source');
  ...
  const processCurrentsForMap = React.useCallback(() => {
    ...
    try {
      ...
      console.error('Error processing current direction data:', error);
      onError?.(error);
      return {
        type: 'FeatureCollection',
        features: []
  ...
  // Add or update the current direction layer
  useEffect(() => {
    ...
    try {
      ...
      console.error('Error adding/updating current direction layer:', error);
      onError?.(error);
    }
    ...
    return () => {
      if (map && map.isStyleLoaded()) {
        try {
          ...
          console.error('Error cleaning up current direction layer:', error);
        }
      }
    };
  }, [map, layerId, sourceId]);

  // Add click handlers for current direction vectors
  useEffect(() => {
    ...
    const handleClick = (e) => {
      if (e.features && e.features.length > 0) {
        ...
        const popupContent = `
          <div class="current-direction-popup">
            <h3 style="margin: 0 0 8px 0; color: #1e40af; font-size: 14px;">Current Direction</h3>
...
export default CurrentDirectionLayer;