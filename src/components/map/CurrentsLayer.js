 ...
 * Renders ocean currents as vector arrows on a Mapbox map
 */
const CurrentsLayer = ({
  map,
  data = [],
  ...
  const layerIdRef = useRef('currents-vectors');
  const sourceIdRef = useRef('currents-source');
  ...
  const processCurrentsForMap = React.useCallback(() => {
    ...
    try {
      ...
      console.error('Error processing currents data:', error);
      onError?.(error);
      return {
        type: 'FeatureCollection',
        features: []
    ...
    return () => {
      if (map && map.isStyleLoaded()) {
        try {
          ...
          console.error('Error cleaning up currents layer:', error);
        }
      }
    };
  }, [map, layerId, sourceId]);

  // Add click handlers for currents vectors
  useEffect(() => {
