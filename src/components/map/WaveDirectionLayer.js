...
/**
 * WaveDirectionLayer Component
 * Renders wave direction as vector arrows on a Mapbox map
 */
const WaveDirectionLayer = ({
  map,
  data = [],
  isVisible = false,
  vectorScale = 0.009,
  colorBy = 'speed',
  depthFilter = null,
  displayParameter = 'Wave Direction',
  maxVectors = 1000,
  minMagnitude = 0,
  onError = null
}) => {
  const layerIdRef = useRef('wave-direction-vectors');
  const sourceIdRef = useRef('wave-direction-source');
  ...
  const processCurrentsForMap = React.useCallback(() => {
    ...
    try {
      ...
      console.error('Error processing wave direction data:', error);
      onError?.(error);
      return {
        type: 'FeatureCollection',
        features: []
  ...
  // Add or update the wave direction layer
  useEffect(() => {
    ...
    try {
      ...
      console.error('Error adding/updating wave direction layer:', error);
      onError?.(error);
    }
    ...
    return () => {
      if (map && map.isStyleLoaded()) {
        try {
          ...
          console.error('Error cleaning up wave direction layer:', error);
        }
      }
    };
  }, [map, layerId, sourceId]);

  // Add click handlers for wave direction vectors
  useEffect(() => {
    ...
    const handleClick = (e) => {
      if (e.features && e.features.length > 0) {
        ...
        const popupContent = `
          <div class="wave-direction-popup">
            <h3 style="margin: 0 0 8px 0; color: #1e40af; font-size: 14px;">Wave Direction</h3>
...
export default WaveDirectionLayer;