...
/**
 * WindSpeedLayer Component
 * Renders wind speed as vector arrows on a Mapbox map
 */
const WindSpeedLayer = ({
  ...
  const layerIdRef = useRef('wind-speed-vectors');
  const sourceIdRef = useRef('wind-speed-source');
  ...
  const processCurrentsForMap = React.useCallback(() => {
    ...
    console.log('WindSpeedLayer - Processing data:', { 
      ...
      if (!data || data.length === 0) {
        console.log('WindSpeedLayer - No data available');
    ...
    try {
      ...
      console.log('WindSpeedLayer - Generated vectors:', vectorData.features.length);

      return vectorData;
    } catch (error) {
      console.error('Error processing wind speed data:', error);
      onError?.(error);
      return {
  ...
  // Add or update the wind speed layer
  useEffect(() => {
    ...
    try {
      ...
      console.error('Error adding/updating wind speed layer:', error);
      onError?.(error);
    }
    ...
    return () => {
      if (map && map.isStyleLoaded()) {
        try {
          ...
          console.error('Error cleaning up wind speed layer:', error);
        }
      }
    };
  }, [map, layerId, sourceId]);

  // Add click handlers for wind speed vectors
  useEffect(() => {
    ...
    const handleClick = (e) => {
      if (e.features && e.features.length > 0) {
        ...
        const popupContent = `
          <div class="wind-speed-popup">
            <h3 style="margin: 0 0 8px 0; color: #1e40af; font-size: 14px;">Wind Speed</h3>
