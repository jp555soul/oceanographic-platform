...
export const getAIResponse = (message, context) => {
    ...
    if (msg.includes('temperature') || msg.includes('thermal')) {
        if (currentData?.temperature !== null && currentData?.temperature !== undefined) {
            ...
            return `Thermal structure: Water temperature at ${selectedDepth}ft is ${currentData.temperature.toFixed(2)}°C. The vertical gradient suggests ${selectedDepth < 50 ? 'mixed layer' : 'thermocline'} dynamics. This thermal profile influences marine life distribution and acoustic propagation. Temperature anomaly of ${anomaly.toFixed(1)}°C from baseline detected.`;
        } else {
